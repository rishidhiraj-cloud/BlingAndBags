...
const savingCashInput = document.getElementById('savingCash');
...
function init() {
    ...
    savingCashInput.addEventListener('input', calculateAllFields);
...
function calculateAllFields() {
    ...
    const savingCash = parseFloat(savingCashInput.value.replace(/,/g, '')) || 0;
...
async function checkExistingEntry(date) {
    try {
        ...
        if (data && !error) {
            ...
            savingCashInput.value = data.saving_cash || 0;
            calculateAllFields();
...
async function handleSubmit(e) {
    ...
    try {
        ...
        if (cashAmount < 0 || upiAmount < 0 || cardAmount < 0 || savingCash < 0) {
