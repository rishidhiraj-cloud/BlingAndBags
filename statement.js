...
// Saving Cash credit per day = saving_cash from daily_entries.
async function fetchCredits(account, startStr, endStr) {
    const { data, error } = await supabaseClient
        .from('daily_entries')
        .select('date, upi_amount, card_amount, saving_cash')
        .gte('date', startStr)
        .lte('date', endStr)
    ...
    const rows = (data || []).map(e => {
        ...
        } else if (account === 'Saving Cash') {
            amount = parseFloat(e.saving_cash) || 0;
        }
        return {
            date: e.date,
...
async function fetchDebits(account, startIso, nextStartIso) {
    const paidFrom = account === 'BoB' ? 'Bank' : 'Saving Cash';
